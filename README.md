...
>  - leading, trailing = superView 로 부터 30 떨어지도록 세팅
 > - top = superView 로 부터 200 떨어지도록 세팅
  ...
  아! 참고로 코드베이스UI 와 스토리보드 중 두 가지 방법을 모두 구현하면 너무 좋겠지만 코드베이스UI를 먼저 만들어보겠습니다!
...
엄... 왜 제 숫자들이 구석에 들어가있을까요... 심지어 폰트 설정도 안 된 거 같은데... 
--------
...
앗 그리고 제가 튜터님과 화면을 공유하면서 문제됐던 화면 오류도 바로 고쳤습니다.
...
```
topAnchor를 수정했는데 과제 조건과 맞지 않는 숫자라서 마음대로 바꾸는게 마음에 걸립니다
...
안녕하십니까? 레벨3을 붙잡고 있는지 무려 3일 째 되는 날입니다 오늘은 무슨 일이 있어도 레벨4까지 끝내겠습니다. 며칠 동안 만든 거를 갈아엎고 갈아엎고를 반복했는데 아무리 생각해도 제 생각이 잘못 된 거 같아서 그 생각을 다 고치고 고쳐진 생각들로 해보겠습니다. 아무리 생각해도 제가 Lv2에서 나온 조건이랑 Lv3에 나온 조건들을 다 따로따로 만들려고 하다가 생긴 문제 같아요.
...
한 줄로 요약하면 = 너무 길고 귀찮음. 이럴 때 사용하라고 나온게 반복문 아닌가? 라는 생각이 들었습니다. 연산자 버튼들만 오렌지컬러를 넣고 나머지 숫자 버튼들은 원래 색상을 그대로 가져가는 방법으로 가는게 좋을 거 같습니다. 
...
역시 코드는 보고 또 보고 계속 확인해야되는 이유를 이번 과제하면서 또 한 번 느낍니다. 이런식으로 잘못 적고~ 오타나고~ 그런게 한 두번이 아니에요,,, 
...
> -  기본으로 라벨에 노출되어 있던 텍스트 오른쪽에 버튼을 클릭하면 그 버튼의 값이 추가되도록 합니다.
>     - 예를 들어 설명하면
>       1. 맨처음 기본값 `0`
>       2. 그 다음 `1` 클랙했음 -> 표시되는 값은 `01`
>       3. 그 다음 `2` 클랙했음 -> 표시되는 값은 `012`
>       4. 그 다음 `+` 클랙했음 -> 표시되는 값은 `012+`
>       5. 그 다음 `3` 클랙했음 -> 표시되는 값은 `012+3`
> - 하지만 `012`라는 값은 이상합니다. 맨 앞자리가 `0`인 숫자라면, `0`을 지우고 표현하도록 합니다.
